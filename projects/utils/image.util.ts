...
export class ImageUtil {
     ...
     * @param quality           压缩质量
     */
    static compressImages(imageSrc: string[], minPixel: number, imgType: string[],
                          quality?: number): Observable<string[]> {
        let result = new Observable<string[]>(observer => {
            ...
            for (let index = 0; index < imageSrc.length; index++) {
                ...
                img.onload = () => {
                    ...
                    // 取较小的尺寸除以需要的尺寸
                    const scale = b ? img.naturalHeight / minPixel : img.naturalWidth / minPixel;
