...
export class DeviceUtil {
    ...
    public analyzeVersion(ret, deviceItem) {
        if (!('end' in deviceItem.version)) {
            deviceItem.version.end = '';
        }
        if (!('item' in deviceItem.version)) {
            deviceItem.version.item = 1;
        }
        
        let versionRegEx = new RegExp(deviceItem.version.start + '([.0-9a-zA-Z]+)' + deviceItem.version.end, 'i');
        let rep = ret.ua.match(versionRegEx);
    ...
    public analyzeItem(ret, deviceList) {
        let rep = null;
        let fixedName = '';
        let deviceItem = null;
        ...
        for (let i = 0; i < deviceKeys.length; i++) {
            ...
            if (!(rep = ret.ua.match(deviceItem.regEx))) {
                continue;
            }
            
            let deviceItems = Object.keys(deviceItem);
            for (let j = 0; j < deviceItems.length; j++) {
                let singleName = deviceItems[j];
                let singleItem = deviceItem[singleName];
                switch (singleName) {
                    case 'childItem':
                    case 'version':
                    case 'regEx':
                    ...
                    case 'model':
                        ...
                        for (let k = 1; k < rep.length; k++) { // rep[0] is useless
            ...
            let checkVersionSwitch = ('version' in deviceItem);
            if ('childItem' in deviceItem) {
                if (this.analyzeItem(ret, deviceItem.childItem)) {
                    checkVersionSwitch = false;
                }
            }
            
            if (checkVersionSwitch) {
                this.analyzeVersion(ret, deviceItem);
            }
    ...
    public processAnalyze(ret) {
        if (this.analyzeItem(ret, this.deviceList)) {
            // Do nothing
        }
        // Apple
