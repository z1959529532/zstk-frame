 ...
 * @ignore
 */
export class ZAnalyzeBrowserUtil {
    browserList = [
    ...
    browserRegEx = new RegExp(
        this.browserList.concat(Object.keys(this.defaultBrowserList)).join('|'), 'i');
    ...
    public setRetName(ret, rep) {
        ret.image = rep.image;
        ret.full = rep.title.replace(/\{%(.+)%\}/, (match, p1) => {
            return this.getVersion(ret, p1);
        });
    }
    
    public getVersion(ret, title) {
        let lowerTitle = title.toLowerCase();
        let start = lowerTitle;
        ret.name = title;
        
        if ((lowerTitle === 'opera' || lowerTitle === 'opera next' || lowerTitle === 'opera developer') &&
            /OPR/i.test(ret.ua)) {
            start = 'OPR';
        } else if (
            ((lowerTitle === 'opera' || lowerTitle === 'opera next' || lowerTitle === 'opera labs') &&
                /Version/i.test(ret.ua)) ||
            ...
            (lowerTitle === 'tenfourfox' || lowerTitle === 'classilla' || lowerTitle === 'msie') &&
            / rv:([.0-9a-zA-Z]+)/i.test(ret.ua)
        ) {
        ...
        let regEx = new RegExp(start + '[ |/|:]?([.0-9a-zA-Z]+)', 'i');
        let rep = ret.ua.match(regEx);
        if (rep !== null) {
            ret.version = rep[1];
        } else {
            ret.version = '';
        ...
        if (lowerTitle in this.displayNameList) {
            for (let tempVar in this.displayNameList[lowerTitle]) {
                ret[tempVar] = this.displayNameList[lowerTitle][tempVar];
            }
        }
    ...
    public analyze(uaString: string) {
        let ret = {
            ua: uaString,
            name: '',
            version: '',
        ...
        let res = uaString.match(this.browserRegEx);
        let rep = null;
        if (res !== null) {
            ...
            rep = {};
            if (this.defaultBrowserList[name]) {
                Object.keys(this.defaultBrowserList[name]).forEach((key) => {
                    if (key !== 'upper') {
                        rep[key] = this.defaultBrowserList[name][key];
                    } else {
                        upper = this.defaultBrowserList[name].upper;
                    }
                });
            }
            if (!('title' in rep)) {
                if (upper !== null) {
                    let upperTitle = name;
                    upper.forEach((letterId) => {
                        upperTitle = upperTitle.substr(0, letterId) + upperTitle.substr(letterId, 1).toUpperCase() +
                            upperTitle.substr(letterId + 1, upperTitle.length); // fuck IE
                    });
                    rep.title = '{%' + upperTitle + '%}';
                } else {
                    rep.title = '{%' + name.toLowerCase().replace(/[a-z]/, (m) => {
            ...
            this.setRetName(ret, rep);
        } else if (/QQ(?!Download|Pinyin)/.test(ret.ua)) {
            ret.full = this.getVersion(ret, 'QQ');
            ret.image = 'qq';
        } else if (/Galaxy/i.test(ret.ua) && !/Chrome/i.test(ret.ua)) {
            ret.full = this.getVersion(ret, 'Galaxy');
            ret.image = 'galaxy';
        } else if (/Opera Mini/i.test(ret.ua)) {
            ret.full = this.getVersion(ret, 'Opera Mini');
            ret.image = 'opera-2';
        } else if (/Opera Mobi/i.test(ret.ua)) {
            ret.full = this.getVersion(ret, 'Opera Mobi');
            ret.image = 'opera-2';
        } else if (/Opera/i.test(ret.ua)) {
            ret.full = this.getVersion(ret, 'Opera');
            ret.image = 'opera-1';
            ...
            if (/(Edition Next)/i.test(ret.ua)) {
                ret.full = this.getVersion(ret, 'Opera Next');
                ret.image = 'opera-next';
            } else if (/(Edition Developer)/i.test(ret.ua)) {
                ret.full = this.getVersion(ret, 'Opera Developer');
            ...
            ret.name = 'Safari';
            if (/Version/i.test(ret.ua)) {
                ret.full = this.getVersion(ret, 'Safari');
            } else {
                ret.full = ret.name;
            }
            ...
            ret.full = this.getVersion(ret, 'Firefox');
            ret.image = 'firefox';
        } else if (/MSIE/i.test(ret.ua) || /Trident/i.test(ret.ua)) {
            ret.full = 'Internet Explorer' + this.getVersion(ret, 'MSIE');
            ret.image = 'msie';
            rep = ret.ua.match(/(MSIE[ |/]?| rv:)([.0-9a-zA-Z]+)/i);
            if (rep !== null) {
                let ieVersion = parseInt(rep[2], 10);
                if (ieVersion >= 11) {
                    ret.image = 'msie11';
